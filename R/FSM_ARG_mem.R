#' Track memory usage of FSM_ARG.decimal
#'
...
#' @return A matrix about memory usage.
#' @export
#'
#' @examples
#' library(pryr)
#'
#'
#' ARG1 <- FSM_ARG_mem(20L, 0.1, 100L)
#' ARG2 <- FSM_ARG_mem(5L, 0.1, 200L, bacteria = TRUE, delta = 10, optimise_recomb = TRUE, clonal = TRUE)
FSM_ARG_mem <- function(n, rho_site, L, bacteria=FALSE, delta=NULL, node_max=1000,
  ...
  before_mem <- as.numeric(mem_used())/1000000
  mem_record <- c()
  node_record <- c()
  edge_record <- c()
  ...
  mem_record <- c(mem_record, as.numeric(mem_used())/1000000 - before_mem)
  node_record <- c(node_record, node_index)
  edge_record <- c(edge_record, edge_index)
  ...
  mem_mat <- cbind(mem_record, node_record, edge_record)
  return(mem_mat)
}
